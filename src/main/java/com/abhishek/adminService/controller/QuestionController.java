...
public class QuestionController {
    ...
    public ResponseEntity<Question> create(@Valid @RequestBody CreateQuestionRequest req, @RequestHeader(value = "X-User-Id", required = false) String createdBy) {
        ...
        q.setOptions(req.getOptions().stream().map(o -> {
            Question.Option opt = new Question.Option(); opt.setId(o.getId()); opt.setText(o.getText()); return opt;
        }).toList());
    ...
    public ResponseEntity<List<Question>> list(@RequestParam(required = false) String categoryId) {
        List<Question> res = categoryId == null ? questionService.findAll() : questionService.findByCategory(categoryId);
    ...
    public ResponseEntity<Question> update(@PathVariable String id, @Valid @RequestBody CreateQuestionRequest req) {
        Question q = new Question();
        q.setCategoryId(req.getCategoryId()); q.setDifficulty(req.getDifficulty()); q.setText(req.getText());
        q.setOptions(req.getOptions().stream().map(o -> { Question.Option opt = new Question.Option(); opt.setId(o.getId()); opt.setText(o.getText()); return opt; }).toList());
        q.setCorrectOptionId(req.getCorrectOptionId());
